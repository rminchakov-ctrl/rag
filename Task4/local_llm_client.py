    ...
    def generate(self, prompt, max_tokens=500, temperature=0.1, top_p=0.9, stop=None):
        response = self.llm.create_chat_completion(
            messages=[
                {
                    "role": "system", 
                    "content": "Ты всегда завершаешь цепочку рассуждений. Отвечай только на основе контекста. Не копируй вопросы в ответы. Не копируй примеры в ответы."
                },
                {
                    "role": "user",
                    "content": prompt
...
def get_llm_client(model_path: str = "tinyllama-1.1b-chat-v1.0.Q4_K_M.gguf") -> LocalLLMClient:
